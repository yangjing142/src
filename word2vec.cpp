...
//单字表初始化，即初始化每个单词的能量分布表，table在负样本抽样中用到
void InitUnigramTable() 
{
	int a, i;//循环变量

	double train_words_pow = 0;//词的能量总值
	double d1, power = 0.75;

	 //table反映一个词能量的分布，一个词能量越大，所占用的table的位置越多
	table = (int *)malloc(table_size * sizeof(int));//为table分配空间

	for (a = 0; a < vocab_size; a++) //遍历词汇表，统计词的能量总值train_words_pow
		train_words_pow += pow(vocab[a].cn, power);/*？？？为什么这里要给词频计算一个power的次方？？？*/

	i = 0;//遍历词表
	d1 = pow(vocab[i].cn, power) / train_words_pow;//初始化已遍历的词的能量值占总能力值的比例，初始值为第一个词的能量值在总体中的占比

	for (a = 0; a < table_size; a++)//遍历table
	{
		table[a] = i;//当前词表中的词i占用table[a]

		if (a / (double)table_size > d1) 
		{
			i++;//进入词表中的下一个位置
			d1 += pow(vocab[i].cn, power) / train_words_pow;//将当前词的能量值占总能量值得比例加入到已累计词的当前量总值中
		}
